...
# n_genes = 9000
# n_drugs = 700
# n_drugdrug_rel_types = 1400
# gene_net = nx.planted_partition_graph(900, 10, 0.2, 0.05, seed=42)
# print ('OK')
...
val_test_size = 0.05
import utils, config
gene_adj = utils.load_obj(config.PROCESSED_PROTEINPROTEIN)

gene_degrees = np.array(gene_adj.sum(axis=0)).squeeze()
print ('OK')

gene_drug_adj = utils.load_obj(config.PROCESSED_PROTEINDRUG)
drug_gene_adj = utils.load_obj(config.PROCESSED_DRUGPROTEIN)
print ('OK')



drug_drug_adj_list = utils.load_obj(config.PROCESSED_COMBO_ADR)
drug_degrees_list = [np.array(drug_adj.sum(axis=0)).squeeze() for drug_adj in drug_drug_adj_list]
print ('OK')

n_drugs = len(utils.load_obj(config.DRUG_MAP))
n_genes = len(utils.load_obj(config.PROTEIN_MAP))
